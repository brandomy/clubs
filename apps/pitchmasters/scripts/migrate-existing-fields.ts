...
async function importMembers(csvRows: CSVRow[]) {
  console.log(`📝 Importing ${csvRows.length} members with current schema...`);
  ...
  for (const row of csvRows) {
    try {
      ...
      // Create user record
      const userData = {
        email: row.email || `${row.Name.toLowerCase().replace(/\s+/g, '')}.${row.Surname.toLowerCase().replace(/\s+/g, '')}@placeholder.com`,
      ...
      // Create member profile with only existing schema fields
      const profileData = {
        user_id: user.id,
        club_id: CLUB_ID,

        // Toastmasters data (using existing fields)
        ...
        // Bio with location info embedded
        bio: `${row['Job Title']} ${row.Organization ? `at ${row.Organization}` : ''} based in ${row.City}, ${row.Country}. ${parseBoolean(row.Founder) ? 'Entrepreneur and founder. ' : ''}${parseBoolean(row.Rotarian) ? 'Active Rotarian. ' : ''}${row['Officer Role'] ? `Club ${row['Officer Role']}. ` : ''}`.trim(),
        ...
        // Personal goals (include missing data in text format)
        personal_goals: `Member from ${row.City}, ${row.Country}. ${row.Introducer ? `Introduced by ${row.Introducer}. ` : ''}${row.Mentor ? `Mentored by ${row.Mentor}. ` : ''}${row['TM Member Number'] ? `TM# ${row['TM Member Number']}` : ''}`,
        communication_goals: `${row.Type} member${row.Level ? ` at Level ${row.Level}` : ''}. ${parseBoolean(row.DTM) ? 'Distinguished Toastmaster (DTM). ' : ''}`,
        leadership_goals: `${row['Officer Role'] ? `Current role: ${row['Officer Role']}. ` : ''}${row.Team ? `Team: ${row.Team}. ` : ''}`
      };
...
async function validateImport() {
  ...
  if (profiles && profiles.length > 0) {
    ...
    console.log(`   - Name: ${sample.venture_name || 'N/A'}`);
    console.log(`   - Industry: ${sample.industry}`);
    console.log(`   - Bio: ${sample.bio?.substring(0, 100)}...`);
    console.log(`   - Path Level: ${sample.path_level}`);
  }
}

async function main() {
  try {
    console.log('🚀 Starting CSV data migration (existing schema)...\n');
    ...
    console.log('\n📝 Note: Some fields are embedded in text fields due to current schema.');
    console.log('    To use all CSV fields, update the database schema with:');
    console.log('    docs/database/add-csv-fields.sql');
